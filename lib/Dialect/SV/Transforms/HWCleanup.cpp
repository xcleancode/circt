...
private:
  ...
  sv::IfOp tryMergingIfOps(sv::IfOp ifOp, sv::IfOp prevIfOp) {
    ...
    if (ifOp.getCond() == prevIfOp.getCond()) {
...
sv::IfOp HWCleanupPass::hoistIfOpConditions(sv::IfOp ifOp, sv::IfOp prevIfOp) {
  ...
  ConditionPairInformation condPairInfo(ifOp.getCond(), prevIfOp.getCond());
  ...
  auto i1Type = ifOp.getCond().getType();
  ...
  auto merge = [&]() {
    ...
    if (condPairInfo.rhsOperands.empty()) {
      ...
      auto newCond1 =
          generateCondValue(ifOp.getCond().getLoc(), condPairInfo.lhsOperands);
    ...
    if (condPairInfo.lhsOperands.empty()) {
      ...
      auto newCond2 = generateCondValue(prevIfOp.getCond().getLoc(),
                                        condPairInfo.rhsOperands);
    ...
    auto cond = builder.createOrFold<comb::AndOp>(
        ifOp.getLoc(), condPairInfo.commonOperand, false);
...
void HWCleanupPass::runIfOpsToCaseOnBlock(Block &body) {
  ...
  auto tryConstructingCaseZ = [&](Operation *op) {
    ...
    auto casez = builder.create<sv::CaseOp>(
        ...
        [&](size_t caseIdx) -> std::unique_ptr<circt::sv::CasePattern> {
          ...
          auto thePattern = std::make_unique<circt::sv::CaseBitPattern>(
              constant, op->getContext());
          return thePattern;
        });
    ...
    for (unsigned idx : llvm::seq(0ul, caseIntegerAndIfOps.size())) {
      ...
      for (auto ifOp : ifOps) {
        ...
        comb::ICmpOp icmp = cast<comb::ICmpOp>(ifOp.getCond().getDefiningOp());
        ifOp.erase();
        eraseOpWithoutUse(icmp.getRhs().getDefiningOp());
  ...
  for (auto it = body.begin(), end = body.end(); it != end; it++) {
    ...
    auto patternMatchIfOp = [&](Operation *op) {
      ...
      auto icmpOp =
          dyn_cast_or_null<comb::ICmpOp>(ifOp.getCond().getDefiningOp());
      if (!icmpOp || icmpOp.getPredicate() != comb::ICmpPredicate::eq)
        return false;
      auto constant = dyn_cast_or_null<circt::hw::ConstantOp>(
          icmpOp.getRhs().getDefiningOp());
      if (!constant)
        return false;
      ...
      // creating.
      if (!comparedValue)
        comparedValue = icmpOp.getLhs();
      else if (comparedValue != icmpOp.getLhs())
        return false;
    ...
    if (!success && !mlir::isMemoryEffectFree(op))
