...
static void legalizeModuleBody(const LoweringOptions &options,
                               HWModuleOp module,
    ...
    if (!isa<HWModuleOp>(op)) {
      if (auto name = op->getAttrOfType<StringAttr>(verilogNameAttr)) {
        ...
        if (auto labelAttr = op->getAttrOfType<StringAttr>("label"))
          ...
          StringRef str = llvm::TypeSwitch<Operation *, StringRef>(op)
                              .Case<AssertOp, AssertConcurrentOp>(
                                  [](auto op) { return "assert"; })
                              .Case<CoverOp, CoverConcurrentOp>(
                                  [](auto op) { return "cover"; })
...
GlobalNameResolver::GlobalNameResolver(mlir::ModuleOp topLevel,
  ...
  // Legalize module and interface names.
  SmallVector<
      std::pair<HWModuleOp, SmallVector<std::pair<StringAttr, StringRef>>>>
      results;

  for (auto module : topLevel.getOps<HWModuleOp>())
    results.push_back({module, {}});

  mlir::parallelForEach(topLevel.getContext(), results, [&](auto &elem) {
    legalizeModuleBody(options, elem.first, globalNameTable);
  });

  // for (auto [module, names] : results)
  //   for (auto name : names)
  //     globalNameTable.addRenamedParam(module, name.first, name.second);
